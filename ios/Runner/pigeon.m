...
void BiometricsApiSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<BiometricsApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.registration_client.BiometricsApi.setComment"
        binaryMessenger:binaryMessenger
        codec:BiometricsApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(setCommentFieldId:modality:comment:completion:)], @"BiometricsApi api (%@) doesn't respond to @selector(setCommentFieldId:modality:comment:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        NSString *arg_fieldId = GetNullableObjectAtIndex(args, 0);
        NSString *arg_modality = GetNullableObjectAtIndex(args, 1);
        NSString *arg_comment = GetNullableObjectAtIndex(args, 2);
        [api setCommentFieldId:arg_fieldId modality:arg_modality comment:arg_comment completion:^(NSNumber *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    } else {
